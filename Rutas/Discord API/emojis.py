...
def obtener_emoji_paginado(token: str, guild_id: str, page: int):
...
def get(guild_id: str, page: int = Query(0, ge=0), token: str = Header(None)):
    if token is None:
        return {"error": "Se requiere el token del bot en el encabezado"}
    
    emoji_paginado = obtener_emoji_paginado(token, guild_id, page)
